...
import ContactBook, { ContactBookUI, createContact, findNearDuplicates, getFilterDataFromSearch } from './ContactBook.jsx'
...
describe('ContactBookUI', () => {
  ...
  it('shows ContactDetails when a contact is selected', () => {
    ...
    it('render search', () => {
        ...
        const data = { contacts }
        const wrap = shallow(<ContactBookUI {...props} data={data} />)
        const contactSearchSection = wrap.find('.ContactSearch')
        expect(contactSearchSection).toExist()
        expect(contactSearchSection.find('label')).toHaveText('Search: ')
        expect(contactSearchSection.find('input')).toExist()
    })
})
...
// added
describe('get filter data works', () => {
    const data = {
        contacts: [
            { id: '1', name: 'One', address: 'Onestr. 1', postalCode: '11111', city: 'Milano' },
            { id: '2', name: 'Two', address: 'Twostr. 2', postalCode: '22222', city: 'Milano' }
    ...
    let filterData = getFilterDataFromSearch(data, 'One')

    expect(filterData.length).toEqual(1)
    expect(filterData[0].id).toEqual('1')
    filterData = getFilterDataFromSearch(data, 'x')
    expect(filterData.length).toEqual(0)
    filterData = getFilterDataFromSearch(data, 'Milano')
    expect(filterData.length).toEqual(2)
    expect(filterData[0].id).toEqual('1')
    expect(filterData[1].id).toEqual('2')
})
